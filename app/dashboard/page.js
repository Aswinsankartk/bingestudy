  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-8 py-12">
        ...
        <p className="text-gray-400 mb-8">Create a new group or join one with an invite code.</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-gray-200 rounded-2xl">
            ...
            <p className="text-gray-300 text-sm mt-1">Create one or ask a friend for their invite code.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {groups.map((membership) => (
              <div
                ...
                className="border border-gray-200 rounded-2xl p-6 cursor-pointer hover:border-black transition-all duration-200"
              >
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-bold text-black text-lg">{membership.groups.name}</h3>
                  <span className="text-xs font-medium bg-gray-100 text-gray-500 px-2 py-1 rounded-lg">
                    {membership.role}
                  </span>
                </div>
                {membership.groups.subject && (
                  <p className="text-gray-400 text-sm mb-3">{membership.groups.subject}</p>
                )}
                <p className="text-xs text-gray-300 font-mono">Code: {membership.groups.code}</p>
              </div>
            ))}
          </div>
        )}
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4 z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md">
            <h3 className="text-xl font-black text-black mb-6">Create a Group</h3>
      ...
      {showJoin && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4 z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md">
            <h3 className="text-xl font-black text-black mb-6">Join a Group</h3>
