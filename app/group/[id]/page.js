...
export default function GroupRoom() {
  ...
  const handleMemberAction = async (targetUserId, action) => {
    ...
    if (res.ok) {
      await fetchMembers(); // refetch after every action
    } else {
      const data = await res.json();
      alert(data.error);
    }
    ...
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-black text-base leading-tight">
              {group.name}
            </h1>
            {group.subject && (
          ...
          >
            {aiMessages.length === 0 ? (
              ...
              aiMessages.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-xs md:max-w-lg px-4 py-3 rounded-2xl text-sm ${
                      msg.role === "user"
                        ? "bg-black text-white rounded-br-sm"
                        : "bg-gray-100 text-black rounded-bl-sm"
                    }`}
